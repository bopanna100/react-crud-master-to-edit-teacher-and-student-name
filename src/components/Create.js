...
class Create extends Component
{
    ...
    checkValidation(fields) {
    	var error = {};
    	if(fields.studentname.length === 0) {
    		error.studentname = ['This field is required!'];
    	}
    	if(fields.teacher_name.length === 0) {
    		error.teacher_name= ['This field is required!'];
    	}
		...
		if(fields.studentname.length === 0 || fields.teacher_name.length === 0 || fields.email.length === 0) {
			return true;
		} else {
			return false;
		}
			        ...
			        <form onSubmit={this.handleInsertUser}>
			      		<div className="modal-body">
			          		<div className="form-group">
			            		...
			            		<input type="text" className={`form-control form-control-sm ${this.hasErrorFor('studentname') ? 'is-invalid' : ''}`}
			            		 id="studentname" name="studentname" placeholder="student name" onChange={this.handleInputFieldChange} value={this.state.studentname}/>
			            		{this.renderErrorFor('studentname')}
			         	 	</div>
			          		<div className="form-group">
			            		<label htmlFor="teacher_name" className="col-form-label">Teacher Name:</label>
			            		<input type="text" className={`form-control form-control-sm ${this.hasErrorFor('teacher_name') ? 'is-invalid' : ''}`}
			            		 id="teacher_name" name="teacher_name" placeholder="Teacher Name" onChange={this.handleInputFieldChange} value={this.state.teacher_name}/>
			            		{this.renderErrorFor('teacher_name')}
			          		</div>
			          		<div className="form-group">
