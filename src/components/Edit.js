...
class Edit extends Component
{
	...
	UNSAFE_componentWillReceiveProps(user_data) {
		this.setState({
			...
			studentname  : user_data.user.studentname,
			teacher_name: user_data.user.mobile_no,
	...
	handleUpdateUser(e) {
		...
		const data = {
			...
			teacher_name: this.state.mobile_no,
    ...
    checkValidation(fields) {
		...
		if(fields.studentname.length === 0 || fields.mobile_no.length === 0 || fields.email.length === 0) {
			return true;
		} else {
			return false;
		}
			...
			<div className="modal fade" id="editModal" tabIndex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			  	<div className="modal-dialog" role="document">
			        ...
			        <form onSubmit={this.handleUpdateUser}>
						...
						" onChange={this.handleInputFieldChange} value={this.state.studentname}/>
			            		{this.renderErrorFor('studentname')}
			         	 	</div>
			          		<div className="form-group">
			            		<label htmlFor="mobile_no" className="col-form-label">Teacher Name:</label>
			            		<input type="text" className={`form-control form-control-sm ${this.hasErrorFor('mobile_no') ? 'is-invalid' : ''}`}
			            		 id="mobile_no" name="mobile_no" placeholder="Teacher Name" onChange={this.handleInputFieldChange} value={this.state.mobile_no}/>
